...
{% assign pub = r.publication | default: r.source %}{% assign by = r.authors | array_to_sentence_string %}{% if r.authors.size > 0 and by != pub %}{{ by | escape }} in {% endif %}<i>{{ pub | escape }}</i>.<br>
